...
> **이 문서는 UI 가 있는 프로젝트에서만 사용합니다.**
> CLI / 라이브러리 / 백엔드 서비스라면 이 파일은 무시하거나 삭제해도 됩니다.
...
| 금지 | 이유 |
|------|------|
| `backdrop-filter: blur()` | glass morphism 은 AI 템플릿의 가장 흔한 징후 |
| 그라데이션 텍스트 (배경 그라데이션 + bg-clip-text) | AI 가 만든 SaaS 랜딩의 1번 특징 |
| "Powered by AI" 배지 | 기능이 아니라 장식. 사용자에게 가치 없음 |
| box-shadow 글로우 애니메이션 | 네온 글로우 = AI 슬롭 |
| 모든 카드에 동일한 `rounded-2xl` | 균일한 둥근 모서리는 템플릿 느낌 |
| 배경 gradient orb (`blur-3xl` 원형) | 모든 AI 랜딩 페이지에 있는 장식 |

> **참고**: 색상 클리셰 (보라/인디고) 는 *AI 스타일 회피* 관점의 가이드입니다.
> 회사 브랜드 가이드(예: IBM Blue)가 우선이라면 이 항목은 무시하세요.
